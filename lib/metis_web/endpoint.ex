defmodule MetisWeb.Endpoint do
  ...
  plug(MetisHealth.Router)
  plug(MetisGraphQL.Router)
  plug(:halt_if_sent)
  ...
  def init(_key, config) do
    if config[:load_from_system_env] do
      port = Application.get_env(:metis, __MODULE__)[:http][:port] || raise "expected the PORT environment variable to be set"
  ...
  defp ping(%{request_path: "/ping"} = conn, _opts) do
    version = Application.get_env(:metis, :version)
  ...
  defp canonical_host(conn, _opts) do
    opts = PlugCanonicalHost.init(canonical_host: Application.get_env(:metis, :canonical_host))
  ...
  defp force_ssl(conn, _opts) do
    if Application.get_env(:metis, :force_ssl) do
  ...
  defp cors(conn, _opts) do
    opts = Corsica.init(Application.get_env(:metis, Corsica))

    Corsica.call(conn, opts)
  end

  defp basic_auth(conn, _opts) do
    basic_auth_config = Application.get_env(:metis, :basic_auth)
